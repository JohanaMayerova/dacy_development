...
# adding difference in naming, so it is more flexible for differently named conllu files
def _add_sent_id(docs, split, diff, dataset):
    path = assets_path / dataset / f"{diff}{split}.conllu" if diff else assets_path / dataset / f"{split}.conllu"
...
def load_dane():
    ...
    for split in ["train", "dev", "test"]:
        ...
        docs = [e.reference for e in examples]
        dane[split] = docs
        _add_sent_id(docs, split, diff="", dataset="dane")
    return dane
...
# this part does not work for me yet - assertion error, since some qid does not start with Q (-), some have multiple QIDs or is not in entities
# def add_qid(docs):
